...
WHERE  inducted_by IS NOT NULL and inducted_by <> '' AND isapproved= 1 AND project_id='".$_SESSION['admin']."' order by tbl_induction_register.id desc");
?>
...
<body onbeforeunload=”HandleBackFunctionality()” style="background-color: white">
    ...
    width: 65%">
    		...
    		<div class="Form_name" style="text-shadow:1px 1px 1px rgba(18,18,18,1);font-weight:normal;color:#6495ED	 ;letter-spacing:1pt;word-spacing:0pt;font-size:23px;text-align:center;font-family:verdana, sans-serif;line-height:2; background-color: #696969">RESUBMITTED SITE INDUCTION FORMS 
			</div>
        ...
        <tr style="background-color: #696969; color: white">
         ...
         <tbody style="" id='user'>
              ...
              <tr style="background-color: #F0FFFF; font-size: 15px"> 
                ...
                <td>
                  <form id="resubmitted_form<?=$row->id?>" method="post" action="site_induction_resubmitted.php" >
                  <input type="hidden"  name="resubmitted_form" value="<?=$row->id?>" />
