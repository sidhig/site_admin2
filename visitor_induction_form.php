...
if(isset($_POST['back']))
{
 ...
 $get_query=$conn->query("select tbl_employee.id, tbl_employee.given_name as name,tbl_employee.pin,tbl_employee.email_add,tbl_employee.contact_phone,tbl_employer.company_name, tbl_visitor_induction.induction_no,tbl_visitor_induction.id as visit_id,tbl_visitor_induction.position,tbl_visitor_induction.today_date,tbl_visitor_induction.visitor_business,tbl_visitor_induction.visitor_name,tbl_visitor_induction.visitor_mobile,tbl_visitor_induction.visit_reason,tbl_visitor_induction.visit_in ,tbl_visitor_induction.visit_out  FROM tbl_employee JOIN tbl_visitor_induction ON tbl_employee.id = tbl_visitor_induction.induction_no JOIN tbl_induction_register ON tbl_employee.id = tbl_induction_register.id JOIN tbl_employer ON tbl_employer.id = tbl_induction_register.empid WHERE tbl_visitor_induction.induction_no ='1' AND tbl_visitor_induction.id= '".$id."'   AND visit_out !='' ORDER BY created DESC")->fetch_object();
...
<div class="form-group">
  ...
  <div class="col-md-4">
    <input type="submit" class="btn btn-warning" value="Back to Visitor Induction Forms" name="back"> <span class="glyphicon glyphicon-share-alt"></span></input>
  </div>
</div>
